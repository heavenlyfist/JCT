...
router.post('/title', async (req, res) => {
  try {
    ...
    if (validUser) {
      const compositions = await Composition.find({ user: validUser._id, title: { $regex: new RegExp(req.body.query, 'i') } }).sort({ start: -1 });
    }

    // Search for any compositions.
    else {
      const compositions = await Composition.find({ private: false, title: { $regex: new RegExp(req.body.query, 'i') } }).sort({ start: -1 });
    }
...
router.post('/composer', async (req, res) => {
  try {
    ...
    if (validUser) {
      const compositions = await Composition.find({ user: validUser._id, composer: { $regex: new RegExp(req.body.query, 'i') } }).sort({ start: -1 });
    }

    // Search all compositions.
    else {
      const compositions = await Composition.find({ private: false, composer: { $regex: new RegExp(req.body.query, 'i') } }).sort({ start: -1 });
...
router.post('/performer', async (req, res) => {
  try {
    ...
    if (validUser) {
      const compositions = await Composition.find({ user: validUser._id, performers: { $in: [new RegExp(req.body.query, 'i')] } }).sort({ start: -1 });
    }

    else {
      const compositions = await Composition.find({ private: false, performers: { $in: [new RegExp(req.body.query, 'i')] } }).sort({ start: -1 });
...
router.post('/tags', async (req, res) => {
  try {
    ...
    if (validUser) {
      compositions = await Composition.find({ user: validUser._id, tags: { $in: [new RegExp(req.body.query, 'i')] } }).sort({ start: -1 });
    }
    // Search for any compositions.
    else {
      const compositions = await Composition.find({ private: false, tags: { $in: [new RegExp(req.body.query, 'i')] } }).sort({ start: -1 });
    }
...
router.get('/newest/:id', async (req, res) => {
  try {
    ...
    const composition = await Composition.findOne({ user: req.params.id }).sort({ $natural: -1 });
...
router.get('/:id', async (req, res) => {
  try {
    const composition = await Composition.find({ _id: req.params.id, private: false }).sort({ start: -1 });
...
router.get('/user/:id', async (req, res) => {
  try {
    const composition = await Composition.find({ user: req.params.id, private: false }).sort({ start: -1 });
    res.json(composition);
  }
